			...
			include('switchstatus.php');
				...
				if($row["status"] == 3){
					echo'
							...
							<a href="verifikasibukti1.php?idtransaksi='.$idtransaksi.'&status=4"> <button class="btn btn-primary" onclick="return confirm()"> Buang_Benang </button>  </a>
							</div>
					';
				}
				if($row["status"] == 4){
					echo'
							<div class="btn-group mr-2" role="group" >
							<a href="verifikasibukti1.php?idtransaksi='.$idtransaksi.'&status=5"> <button class="btn btn-primary" onclick="return confirm()"> Packing </button>  </a>
							</div>
					';
				}
